  ...
  return (
    <div className="fixed inset-0 bg-white/10 flex justify-center items-center z-50 px-4" style={{ backdropFilter: 'blur(2px)', WebkitBackdropFilter: 'blur(2px)' }}>
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md space-y-4">
        ...
        <button
          className={`w-full border rounded-lg px-4 py-3 text-left font-medium transition ${
            timeOption === 'now' ? "bg-blue-50 border-blue-600 text-black" : "bg-[#FCFCFC] border-gray-200 text-gray-700"
          }`}
          onClick={() => { setTimeOption('now'); setError(''); }}
        >
          Sekarang
        </button>
        ...
        >
          ...
          {timeOption === 'custom' && (
            <input
              ...
              style={{ height: '2.25rem' }} // biar sejajar dengan tombol
            />
          )}
        </button>
        ...
        <div className="flex justify-between gap-3 w-full mt-4">
          <button
            onClick={onCancel}
            className="flex h-10 px-2 flex-1 justify-center items-center gap-1 rounded-lg bg-white shadow-[0px_4px_4px_-0.5px_rgba(0,0,0,0.04),0px_0px_0px_1px_#E0E0E0,0px_2px_4px_0px_rgba(0,0,0,0.04)] text-black text-sm font-medium hover:bg-gray-100 transition"
          >
            Kembali
          </button>
          <button
