...
export default function OrderTypeStep({ onContinue, onCancel }) {
  ...
  return (
    <div className="fixed inset-0 bg-white/10 flex justify-center items-center z-50 px-4" style={{ backdropFilter: 'blur(2px)', WebkitBackdropFilter: 'blur(2px)' }}>
      ...
      >
        <h2 className="text-lg font-semibold">Tipe Pemesanan</h2>
        <div className="relative w-full">
          <select
            ...
            className={`w-full rounded-md border border-[#D0D0D0] bg-[#FCFCFC] px-4 py-3 pr-12 focus:outline-none text-gray-900 appearance-none ${
              orderType ? 'bg-blue-50' : 'bg-[#FCFCFC]'
            } ${orderTypeError ? 'border-red-500' : ''}`}
            style={{
              WebkitAppearance: 'none',
            }}
          >
            <option value="" disabled hidden>Pilih tipe pemesanan</option>
          ...
          />
          {orderTypeError && (
            <span className="text-xs text-red-500 mt-1 absolute left-0 -bottom-5">Tolong Pilih Tipe Pemesanan</span>
          )}
        </div>
        <div className="flex justify-between gap-3 w-full mt-4">
          <button onClick={onCancel} className="flex h-10 px-2 flex-1 justify-center items-center gap-1 rounded-lg bg-white shadow-[0px_4px_4px_-0.5px_rgba(0,0,0,0.04),0px_0px_0px_1px_#E0E0E0,0px_2px_4px_0px_rgba(0,0,0,0.04)] text-red-500 text-sm font-medium hover:bg-red-50 transition">Batal</button>
