...
// Tambahkan useCallback jika ada fungsi internal yang kompleks dan digunakan di dependency array useEffect
...
export default function FormProductUser({
  productData,
  onClose, // onClose dari parent (ProductManagementPage) sudah di-memoize
  onSaveSuccess, // onSaveSuccess dari parent juga sudah di-memoize
  formMode,
  authUser,
}) {
  ...
  // fetchKategoriList tidak perlu useCallback karena hanya dipanggil di useEffect init
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (formMode === "edit" && productData) {
      ...
      if (editProductId === undefined || editProductId === null || String(editProductId).trim() === "" || String(editProductId).toLowerCase() === "undefined") {
        ...
        if (onClose) onClose(); // Panggil onClose yang sudah di-memoize
        return;
      }
      ...
      setImageFile(null);
      setKategori(""); // Reset dulu, akan diisi oleh effect lain
    } else if (formMode === "add") {
      setNamaProduk(""); setDeskripsiProduk(""); setHarga(""); setStok(""); setImageFile(null); setKategori("");
    }
  }, [productData, formMode, onClose]); // onClose adalah dependensi karena dipanggil
  ...
  useEffect(() => {
    if (formMode === "edit" && productData && !isLoadingKategori && kategoriOptions.length > 0) {
      ...
      // Hanya set jika kategori saat ini kosong, untuk memungkinkan user mengubahnya tanpa ter-override
      if (kategori === "") { 
  ...
  }, [formMode, productData, isLoadingKategori, kategoriOptions]); // Hapus `kategori` dari deps
  ...
  const handleSubmit = useCallback(async (event) => {
    ...
    // Penanganan imageFile untuk API JSON
    // Jika gambar WAJIB untuk produk baru dan API Anda JSON, Anda perlu strategi lain (misal upload terpisah dulu)
    // atau backend Anda bisa menerima image_url null/kosong.
    if (formMode === "add" && !imageFile) {
        ...
        // alert("Gambar produk harus diunggah untuk produk baru."); // Aktifkan jika wajib
        // return; 
    }
    ...
    let payload = {
      ...
      stock: finalStok,
      category: kategori,
      image_url: null, // Default ke null
    };

    if (imageFile) {
      // API JSON tidak bisa handle File object. Kirim nama file sebagai placeholder atau info.
      payload.image_url = `local:${imageFile.name}`; // Ini yang akan disimpan di DB jika backend tidak memprosesnya
      ...
      // Jika edit dan tidak ada file baru dipilih, gunakan image_url yang lama (jika ada & valid string)
    ...
    if (formMode === "edit") {
      ...
      response = await addProduct(payload); // Mengirim objek payload
    ...
    if (response && response.error === false) {
      window.alert(response.message || successMessage); // Prioritaskan pesan dari API jika ada
      ...
      kategoriOptions, /* isLoadingKategori tidak perlu di sini, sudah dicegah di UI */
      onSaveSuccess // onSaveSuccess memoized dari parent
    ]);
